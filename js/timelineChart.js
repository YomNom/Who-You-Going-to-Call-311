 ...
 *
 * Design:
 *   - One line per calendar day connected by a monotone curve
...
class TimelineChart {
  ...
  initVis() {
    ...
    // Read the chart div's own dimensions — not the parent panel
    const container = document.getElementById(vis.config.parentElement);
    const containerW = (container ? container.clientWidth  : null) || 1100;
    const containerH = (container ? container.clientHeight : null) || 110;

    // Margin: no top title (card-label handles that); keep room for axes
    vis.margin = { top: 8, right: 18, bottom: 32, left: 44 };

    vis.containerWidth  = containerW;
    vis.containerHeight = containerH;
    vis.width  = containerW - vis.margin.left - vis.margin.right;
    ...
    // Clip path prevents line/area from overflowing axes during brush
  ...
  renderVis() {
    ...
    // Dim overlays (outside brush selection)
    ...
    // Tooltip via brushG (receives all pointer events over chart area)
    vis.brushG
      .on('mousemove.tooltip', (event) => {
        ...
        vis.areaG.selectAll('.timeline-dot')
          .attr('opacity', pt => pt.day.getTime() === d.day.getTime() ? 1 : 0.2)
          .attr('r',       pt => pt.day.getTime() === d.day.getTime() ? 4 : 2);
      ...
      .on('mouseleave.tooltip', () => {
        vis.areaG.selectAll('.timeline-dot').attr('opacity', 0.45).attr('r', 2);
  ...
  _applyDim(x0, x1) {
    ...
    vis.dimOverlayL.attr('x', 0).attr('width', Math.max(0, x0)).attr('opacity', dimOpacity);
    vis.dimOverlayR.attr('x', x1).attr('width', Math.max(0, vis.width - x1)).attr('opacity', dimOpacity);
  }
